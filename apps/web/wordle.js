(() => {
  ...
  function installLobbySettings() {
    document.querySelectorAll('#create-lobby-form,#lobby-settings-form').forEach(form => {
      ...
      if (!box) {
        ...
        if (anchor) anchor.before(holder); else form.appendChild(holder);
        box = holder;
      }
  ...
  function renderBoard(board, index) {
    ...
    for (let r=0;r<rows;r++) { const entry = board.guesses[r]; for (let c=0;c<5;c++) { const letter = entry?.guess?.[c] || ''; cells += `<div class="wordle-cell ${entry ? cellClass(entry.evaluation[c]) : letter ? 'filled' : 'empty'}">${esc(letter)}</div>`; } }
  ...
  function keyStatuses(boards) {
    const map = {}; for (const l of LETTERS) map[l] = boards.map(board => { let status = 'unused'; for (const entry of board.guesses) for (let i=0;i<5;i++) if (entry.guess[i] === l) { const s=entry.evaluation[i]; if(s==='correct') status='correct'; else if(s==='present' && status!=='correct') status='present'; else if(s==='absent' && status==='unused') status='absent'; } return status; }); return map;
  }
  ...
  function render() {
    const main=document.querySelector('.main'); if(!main || !match || !WORDLE_IDS.has(match.gameId)) return;
  ...
  async function refresh(){const id=sessionStorage.getItem('browser-games-active-match');if(!id)return;try{const r=await api('/api/matches/'+id);if(WORDLE_IDS.has(r.match.gameId)){match=r.match;render()}}catch{match=null}}
  new MutationObserver(()=>{style();installLobbySettings(); if(!match){const id=sessionStorage.getItem('browser-games-active-match');if(id)api('/api/matches/'+id).then(r=>{if(WORDLE_IDS.has(r.match.gameId)){match=r.match;render()}}).catch(()=>{});} }).observe(document.body,{childList:true,subtree:true});
